...
pushd "$BASE_DIR" > /dev/null
    echo "Executing packages build steps for Vigil Local..."
    ...
    if [ $rc -eq 0 ]; then
        echo "Success: Done executing packages build steps for Vigil Local"
    else
        echo "Error: Failed executing packages build steps for Vigil Local"
    fi
popd > /dev/null
