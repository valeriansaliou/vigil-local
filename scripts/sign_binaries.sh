...
pushd "$BASE_DIR" > /dev/null
    ...
    if [ $rc -eq 0 ]; then
        echo "Success: Done executing sign steps for Vigil Local v$VIGIL_LOCAL_VERSION"
    else
        echo "Error: Failed executing sign steps for Vigil Local v$VIGIL_LOCAL_VERSION"
    fi
popd > /dev/null
